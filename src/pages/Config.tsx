...
export default function Config() {
  ...
  const handleProvision = async () => {
    if (!appUrl || !apiKey || !clockId) {
      setProvisionMsg({ type: "error", text: "Preencha URL, chave de API e ID do relógio" });
      return;
    }
    setProvisioning(true);
    setProvisionMsg(null);
    try {
      const result = await api.provision({ app_url: appUrl, api_key: apiKey, clock_id: clockId });
      if (result.success) {
    ...
    <div className="px-4 py-6 sm:px-0 space-y-6">
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <Field label="URL do sistema" value={appUrl} onChange={setAppUrl} placeholder="https://sistema.ryanne.com.br" />
